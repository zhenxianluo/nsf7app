...
WORKDIR /edu-design
#增加pgsql的自动密码文件
RUN cp .pgpass ~/ && chmod 600 ~/.pgpass
#更新并安装环境
RUN apt-get update && \
		apt-get install -y vim python python-dev python-pip nodejs npm postgresql-client postgresql curl postgresql-server-dev-all libxml2-dev libxslt1-dev zlib1g-dev && \
